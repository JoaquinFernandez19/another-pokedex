...
import { fetchUserInfo, registerUserInDB, syncStateDataWithDB, shouldResetCheck } from "./firebase/User";

import { addAmountToLocalOwnedPokemonList, checkDevice, fetchPokemons } from "./app-usage/Lib";
...
export const SetAppInitialState = async (authUserObject: UserInfo) => {
  ...
  let ownedPokemons = [];
  if (!recentlyRegister) {
    const ownedPokemonsIdList = userDataDB.catched_pokemons.map((poke: { pokemon_id: number }) => String(poke.pokemon_id));
    ownedPokemons = await fetchPokemonList(ownedPokemonsIdList);
    if (ownedPokemons.length) {
      ownedPokemons = addAmountToLocalOwnedPokemonList(ownedPokemons, userDataDB.catched_pokemons);
    }
  }
  ...
  if (shouldReset) {
    ...
    lastReset = new Date();
    await syncStateDataWithDB(authUserObject, credits, pokemonCollection, lastReset.toString(), userDataDB.catched_pokemons);
  } else {
